...
pub fn solve(input: &str) -> String {
    ...
    moves.iter().for_each(|(repeat, from, to)| {
        let departure_stack = stacks.get_mut(*from - 1).unwrap();

        let content_to_move: Vec<_> = departure_stack.drain(0..(*repeat)).collect();

        let arrival_stack = stacks.get_mut(*to - 1).unwrap();

        content_to_move.iter().rev().for_each(|item| {
            arrival_stack.push_front(*item);
        });
    });
...
mod tests {
    ...
    fn given_input() {
        ...
        assert_eq!(solve(&input), "FSZWBPTBG");
    }
}
